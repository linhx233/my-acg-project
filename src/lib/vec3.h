...
class vec3 {
  public:
	...
	vec3(double e0): e0(e0), e1(e0), e2(e0){}
	vec3(const aiVector3D& v): e0(v.x), e1(v.y), e2(v.z){}
	vec3(const aiColor3D& v): e0(v.r), e1(v.g), e2(v.b){}
	...
	vec3& operator/=(double t){ e0/=t,e1/=t,e2/=t;return *this; }
	inline vec3 operator+(const vec3& v)const{ return vec3(e0+v.e0,e1+v.e1,e2+v.e2);}
	inline vec3 operator-(const vec3& v)const{ return vec3(e0-v.e0,e1-v.e1,e2-v.e2);}
	inline vec3 operator*(const vec3& v)const{ return vec3(e0*v.e0,e1*v.e1,e2*v.e2);}
	inline vec3 operator/(const vec3& v)const{ return vec3(e0/v.e0,e1/v.e1,e2/v.e2);}
...
inline vec3 max(const vec3& u, const vec3& v){return vec3(std::max(u.e0,v.e0),std::max(u.e1,v.e1),std::max(u.e2,v.e2));}
...
class vec3i{
  public:
	...
	vec3i(const aiFace& f){
		if(f.mNumIndices!=3){
			std::clog<<"Triangulate first!"<<std::endl;
			exit(0);
		}
		x=f.mIndices[0],y=f.mIndices[1],z=f.mIndices[2];
	}
};
